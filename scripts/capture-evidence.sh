...
# Clear previous runs but keep the index.
mkdir -p evidence
...
say "2. replay — deterministic, no model, typed outputs"
...
say "3. replay — business outcome: no such member (an answer, not a failure)"
...
say "4. replay — recoverable condition: an unexpected acknowledgement screen"
...
say "5. replay — session expiry: re-authenticate via a nested capability, then restart"
...
say "6. replay — hard failure: the application itself errors"
...
say "7. replay — invalid input, rejected before the browser opens"
...
say "8. escalation — operator approves an irreversible posting"
...
say "9. escalation — operator takes over the live session and posts it themselves"
...
say "10. escalation — the unreviewed draft gets stuck; operator recovers by hand"
...
say "11. cross-tenant — the same capability against a second institution"
